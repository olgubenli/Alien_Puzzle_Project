# -*- coding: utf-8 -*-
"""
Created on Sun Nov  9 15:06:01 2025
...
# === YENİ FONKSİYON: 2. ve 3. DURAK ANALİZİ ===
def analyze_hub_connections(hub_to_spokes):
    """
    Aynı '2. Durak' (Merkez) üzerinden bağlanan '3. Durak' (Aktarma) 
    asteroidleri arasındaki 'Serbest Uçuş' maliyetlerini analiz eder.
    """
    print("\n\n--- 2. VE 3. DURAKLAR ARASI İÇ BAĞLANTI ANALİZİ (YENİ RAPOR) ---")
    print("Aynı '2. Durak' (Merkez) üzerinden bağlanan '3. Durak' (Aktarma) asteroidleri arasındaki 'Serbest Uçuş' maliyetleri.")
    
    # Hub'ları (2. Durakları) alfabetik sırala
    for hub, spokes in sorted(hub_to_spokes.items()):
        # Bu merkeze bağlı 3. durakların benzersiz (unique) listesini al
        unique_spokes = list(set(spokes))
        
        # Eğer bu merkeze 2'den az 3. durak bağlıysa (yani karşılaştırılacak bir şey yoksa) atla
        if len(unique_spokes) < 2:
            continue
        
        print(f"\n--- MERKEZ (2. Durak): {hub} ---")
        print(f"  Bu Merkeze Bağlı 3. Duraklar: {', '.join(unique_spokes)}")
        
        connections = []
        # Bu 3. durakların kendi aralarındaki tüm ikili maliyetlerini hesapla
        for i in range(len(unique_spokes)):
            for j in range(i + 1, len(unique_spokes)):
                ast1 = unique_spokes[i]
                ast2 = unique_spokes[j]
                
                # calculate_cost, ASTEROID_DB, ve WEIGHTS global olduğu için direkt kullanabiliriz
                if ast1 not in ASTEROID_DB or ast2 not in ASTEROID_DB:
                    continue 
                    
                cost = calculate_cost(ast1, ast2, WEIGHTS) 
                connections.append((cost, ast1, ast2))
                
        if not connections:
            print("  (3. Duraklar arasında hesaplanabilir bağlantı bulunamadı)")
            continue
            
        # Bulunan bağlantıları maliyete göre sırala
        sorted_connections = sorted(connections, key=lambda x: x[0])
        
        print("  Bu 3. Duraklar Arasındaki En Verimli İç Bağlantılar (TOP 5):")
        for cost, ast1, ast2 in sorted_connections[:5]:
            print(f"    -> {ast1} <-> {ast2} (Maliyet: {cost:.4f})")
# === YENİ FONKSİYON SONU ===
...
def main():
    ...
    # === YENİ: Hub->Spoke (2.->3. Durak) Haritası Oluşturma ===
    hub_to_spokes = {}
    for m_name, m_data in ALL_MATCHES_DATA.items():
        for path in m_data.values():
            # Yolun en az 3 elemanı olmalı (399 -> Hub -> Spoke)
            if len(path) >= 3: 
                hub = path[1]   # 2. Durak (Merkez)
                spoke = path[2] # 3. Durak (Aktarma)
                
                # Sadece DB'de olanları dikkate al
                if hub not in ASTEROID_DB or spoke not in ASTEROID_DB:
                    continue 
                
                if hub not in hub_to_spokes:
                    hub_to_spokes[hub] = []
                hub_to_spokes[hub].append(spoke)
    # ========================================================
    ...
    # === YENİ RAPOR ÇAĞRISI: 2. ve 3. DURAK ANALİZİ ===
    # 'hub_to_spokes' sözlüğü main() içinde daha önce oluşturulmuştu
    analyze_hub_connections(hub_to_spokes)
