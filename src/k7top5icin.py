# -*- coding: utf-8 -*-
"""
Created on Mon Nov  3 17:19:14 2025
...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov  2 21:55:00 2025
@author: olgub

--- UZAYLI BULMACASI FİNAL MOTORU (v4.2_k7 - FİNAL (TOP 5 ANALİZİ)) ---
Analiz: Bu kod, v4.2'nin sağlam mantığını kullanır.
AMAÇ: 'k=7' (Tüm 56 eşleşme) durumunu izole bir şekilde test etmek.
GÜNCELLEME: 'main' fonksiyonu artık ana hedefler
            için "Top 5" en verimli rotayı bulmak için
            bir 'pruning' (budama) döngüsü içeriyor ve
            bu raporu konsolun en sonunda gösteriyor.
"""

import heapq
import copy  # <-- "TOP 5" ANALİZİ İÇİN EKLENDİ

# --- KULLICI AYARLARI VE VERİ GİRİŞİ ---
WEIGHTS = { 'wa': 1.0, 'we': 5.0, 'wi': 10.0, 'wT': 2.0 }

# === k=7 (TÜM 56 EŞLEŞME) VERİSİ ===
# (İsimler final veriyle (k=7) eşleşmesi için düzeltildi)
ALL_MATCHES_DATA = {
...
def generate_asteroid_metro_map(final_routes, asteroid_graph, asteroid_to_match):
    ...
    print("Harita, ana kapılara giden en verimli (Top 1) rotaları gösterir.")
...
def main():
    ...
    print("Uzaylı Bulmacası Motoru (v4.2_k7 - 56 Eşleşme Final TOP 5 ANALİZİ)...")
    ...
    # Bu bizim ana "matrisimiz".
    original_asteroid_graph = {ast: {} for ast in all_known_asteroids}

    # === TAM FİZİKSEL MODEL (Eşleşme İçi Rotalar) ===
    for m_name, m_data in ALL_MATCHES_DATA.items():
        for path in m_data.values():
            # DB isimlerini düzelt (Quientero -> Quintero, Joycekong -> Joycekang)
            fixed_path = []
            path_ok = True
            for ast in path:
                if ast == "26940 Quientero":
                    fixed_path.append("26940 Quintero")
                elif ast == "30030 Joycekong":
                    fixed_path.append("30030 Joycekang")
                elif ast not in ASTEROID_DB:
                    print(f"UYARI: {m_name} atlanıyor. DB'de yok: {ast}")
                    path_ok = False
                    break
                else:
                    fixed_path.append(ast)
            
            if not path_ok:
                continue
                
            path = fixed_path # fixed_path'i kullan
            
            for i in range(len(path) - 1):
                from_ast, to_ast = path[i], path[i+1]
                cost = calculate_cost(from_ast, to_ast, WEIGHTS)
                original_asteroid_graph[from_ast][to_ast] = cost
        ...
        for j in range(i + 1, len(match_names)):
            ...
            for ast1 in m1_asteroids:
                for ast2 in m2_asteroids:
                    ...
                    if ast2 not in original_asteroid_graph[ast1] or cost < original_asteroid_graph[ast1][ast2]:
                        original_asteroid_graph[ast1][ast2] = cost
                        original_asteroid_graph[ast2][ast1] = cost

    print("Dijkstra (Top 1) çalıştırılıyor...")
    
    if '399' not in original_asteroid_graph:
        print("KRİTİK HATA: '399' (Dünya) grafikte bulunamadı.")
        return
        
    # === ANA ROTA KEŞFİ (TOP 1) - Bu bölüm Metro Haritası için ===
    distances, previous_nodes = dijkstra(original_asteroid_graph, '399')
    final_routes_for_map = {}
    print("\n--- ANA ROTA KEŞİF SONUÇLARI (Top 1) ---\n")
    for target_id, target_desc in TARGET_ASTEROIDS.items():
        ...
        if target_id not in previous_nodes or cost == float('inf'):
              print(f"  -> {target_id} için rota bulunamadı.")
              continue
    ...
    for m_name, m_data in sorted(ALL_MATCHES_DATA.items()):
        ...
        for start_asteroid in start_asteroids_in_match:
            ...
            destinations = []
            if start_asteroid in original_asteroid_graph: # Orijinal grafiği kullan
                for target_asteroid, cost in original_asteroid_graph.get(start_asteroid, {}).items():
    ...
    # --- METRO HARİTASI (NORMAL ÇIKTI) ---
    generate_asteroid_metro_map(final_routes_for_map, original_asteroid_graph, asteroid_to_match)


    # === GÜNCELLENMİŞ BÖLÜM: TOP 5 ROTA ANALİZİ (KODUN EN SONUNDA) ===
    print("\n" + "="*50)
    print("--- TOP 5 EN VERİMLİ ROTA ANALİZİ (k=7 - FİNAL) ---")
    print("En iyi rotayı (Top 1) 'budayarak' alternatif transit yolları aranıyor...")
    
    for target_id, target_desc in TARGET_ASTEROIDS.items():
        print(f"\n[*] Hedef: {target_id} ({target_desc}) için Top 5 Rota:")
        
        # Orijinal grafiği her hedef için sıfırla
        temp_graph_for_target = copy.deepcopy(original_asteroid_graph)
        
        # En iyi 5 rotayı bulmak için döngü
        for i in range(5):
            # 1. O anki (budanmış) grafikte en iyi yolu bul
            distances_top_n, previous_nodes_top_n = dijkstra(temp_graph_for_target, '399')
            
            cost = distances_top_n.get(target_id, float('inf'))
            
            if cost == float('inf') or target_id not in previous_nodes_top_n or not previous_nodes_top_n.get(target_id):
                print(f"  {i+1}. Rota: Bulunamadı (Tüm yollar tüketildi).")
                break # Daha fazla yol yoksa döngüden çık
                        
            # 2. Yolu (path) oluştur
            path = []; current_node = target_id
            while current_node: 
                path.append(current_node)
                current_node = previous_nodes_top_n.get(current_node)
            path.reverse()
            
            # 3. Yolu ve "Hesaplamaları" (maliyet dökümünü) yazdır
            line = f"  {i+1}. Rota (Maliyet: {cost:.4f}): [DÜNYA]"
            total_cost_check = 0
            
            if len(path) < 2: 
                print(f"  {i+1}. Rota: ROTA YOK (Maliyet: {cost:.4f})")
                continue

            for j in range(len(path) - 1):
                from_ast, to_ast = path[j], path[j+1]
                
                step_cost = temp_graph_for_target.get(from_ast, {}).get(to_ast, 0.0)
                total_cost_check += step_cost
                
                ast_name = f"{to_ast}({asteroid_to_match.get(to_ast, 'Bilinmiyor')})"
                line += f" --({step_cost:.2f})--> [{ast_name}]"
            
            print(line + f"  (Kontrol Toplamı: {total_cost_check:.4f})")

            # 4. Bu rotayı "buda" (Prune): Rotanın ilk durağını (transit hub) sil
            transit_hub = path[1] # 'Dünya'dan sonraki ilk durak
            
            if transit_hub in temp_graph_for_target:
                del temp_graph_for_target[transit_hub] 
                
                for node_key, connections in temp_graph_for_target.items():
                    if transit_hub in connections:
                        del connections[transit_hub]
                                
                print(f"     -> (Transit istasyon '{transit_hub}' bir sonraki arama için kaldırıldı.)")
            else:
                print(f"     -> (Transit istasyon '{transit_hub}' zaten kaldırılmış, döngü sonlanıyor.)")
                break
