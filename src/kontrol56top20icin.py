...
--- UZAYLI BULMACASI FİNAL MOTORU (v4.2_k7 - FİNAL (TOP 20 ANALİZİ)) ---
...
GÜNCELLEME: 'main' fonksiyonu artık ana hedefler
            için "Top 20" en verimli rotayı bulmak için
            bir 'pruning' (budama) döngüsü içeriyor.
"""

import heapq
import copy  # <-- "TOP 20" ANALİZİ İÇİN EKLENDİ
...
def generate_asteroid_metro_map(final_routes, asteroid_graph, asteroid_to_match):
    ...
    print("\n\n--- UZAY YOLU METRO HARİTASI (k=7 / 56 Eşleşme - FİNAL - Top 1) ---")
...
def main():
    # Etiketleri k=7 (56 Eşleşme) ve TOP 20 olarak güncelle
    print("Uzaylı Bulmacası Motoru (v4.2_k7 - 56 Eşleşme Final TOP 20 ANALİZİ)...")
    ...
    # Bu bizim ana "matrisimiz". Buna dokunmayacağız.
    ...
    for m_name, m_data in sorted(ALL_MATCHES_DATA.items()):
        ...
        for start_asteroid in start_asteroids_in_match:
            ...
            if not sorted_destinations:
                ...
                for dest_ast, cost in sorted_destinations[:5]: # Top 5 transferi göster
                    dest_match = asteroid_to_match.get(dest_ast, "Bilinmiyor")
                    print(f"  -> {dest_ast} ({dest_match}): (Maliyet: {cost:.4f})")
            print("")

    # En sonda Top 1 rotaların Metro Haritasını çiz
    generate_asteroid_metro_map(final_routes_for_map, original_asteroid_graph, asteroid_to_match)


    # === YENİ BÖLÜM: TOP 20 ROTA ANALİZİ (k=7 FİNAL) ===
    # BU BLOK İSTEĞİN ÜZERİNE ÇIKTININ EN SONUNA TAŞINDI
    print("\n" + "="*50)
    print("--- FİNAL LİSTESİ: ANA/ARA KAPILAR İÇİN TOP 20 ROTA (k=7) ---")
    print("Metro Haritası'ndaki (Top 1) rotalar 'budanarak' bulunan en verimli 20 alternatif:")
    
    for target_id, target_desc in TARGET_ASTEROIDS.items():
        print(f"\n[*] Hedef: {target_id} ({target_desc}) için Top 20 Rota:")
        
        # Orijinal grafiği her hedef için sıfırla
        temp_graph_for_target = copy.deepcopy(original_asteroid_graph)
        
        # En iyi 20 rotayı bulmak için döngü
        for i in range(20):
            ...
            if len(path) < 2: # Rota sadece Dünya'dan ibaretse (ulaşılamaz)
            ...
            for j in range(len(path) - 1):
                ...
                # Maliyeti o anki 'temp_graph_for_target' grafiğinden al
